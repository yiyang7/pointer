...
def article2ids(article_words, word_to_index):
  ...
  ids = []
  oovs = []
  unk_id = word_to_index[UNKNOWN_TOKEN.lower()]
  for w in article_words:
    i = word_to_index[w.lower()]
    if i == unk_id: # If w is OOV
      if w not in oovs: # Add to list of OOVs
        oovs.append(w)
      oov_num = oovs.index(w) # This is 0 for the first article OOV, 1 for the second article OOV...
      ids.append(len(word_to_index) + oov_num) # This is e.g. 50000 for the first article OOV, 50001 for the second...
    else:
      ids.append(i)
  return ids, oovs


def abstract2ids(abstract_words, word_to_index, article_oovs):
  ...
  for w in abstract_words:
    ...
    if i == unk_id: # If w is an OOV word
      if w in article_oovs: # If w is an in-article OOV
        vocab_idx = len(word_to_index) + article_oovs.index(w) # Map to its temporary article OOV number
...
def outputids2words(id_list, word_to_index, article_oovs):
  ...
  for i in id_list:
    try:
      w = word_to_index[i.lower()] # might be [UNK]
    except ValueError as e: # w is OOV
      assert article_oovs is not None, "Error: model produced a word ID that isn't in the vocabulary. This should not happen in baseline (no pointer-generator) mode"
      article_oov_idx = i - len(word_to_index)
      try:
...
def show_art_oovs(article, word_to_index):
  """Returns the article string, highlighting the OOVs by placing __underscores__ around them"""
  unk_token = word_to_index[UNKNOWN_TOKEN.lower()]
  words = article.split(' ')
  words = [("__%s__" % w) if word_to_index[w.lower()]==unk_token else w for w in words]
  out_str = ' '.join(words)
  return out_str


def show_abs_oovs(abstract, word_to_index, article_oovs):
  ...
  new_words = []
  for w in words:
    if word_to_index[w.lower()] == unk_token: # w is oov
